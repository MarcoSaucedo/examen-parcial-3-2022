...
public class ShoppingCart {
    ...
    public void addItem(Item item) {
        ...
        for (Item existenItem : items) {
            if (existenItem.getCode().equals(item.getCode()) && existenItem.getUnitCost().compareTo(item.getUnitCost()) == 0) {
                existenItem.setQuantity(existenItem.getQuantity() + item.getQuantity());
                return;
            }
        }
         items.add(item);
